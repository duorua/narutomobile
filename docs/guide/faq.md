...
2. 进入福利站并执行喂豚豚。
3. 喂豚豚结束后进入或关闭金币助手。
4. MuMu 专用高速截图连续失败并强制重连。
5. 重连期间截图从 `1920×1080` 切换为 `1080×1920`，随后 MFAAvalonia 退出。

该路径是当前已知的稳定复现方式，但其他需要频繁截图的任务也可能在 MuMu 截图服务异常时触发同一问题。

### 如何确认是同一问题

检查 `debug/maafw.log`，出现以下任一日志通常表示 MuMu 专用高速截图发生异常：
...
`1920×1080` 和 `1080×1920` 之间切换，可能导致 MFAAvalonia 异常退出。Windows 事件查看器通常会记录
`MFAAvalonia.exe` 在 `ucrtbase.dll` 中以异常码 `0xc0000409` 退出。

### 该方案解决什么问题

以下配置会停止使用兼容性较低的 MuMu 专用高速截图，改用通用无损 ADB 截图，避免截图服务重连时的横竖屏尺寸切换
继续触发 MFAAvalonia 原生崩溃。该方案是稳定规避措施，不会修改情报社任务逻辑，也不等同于修复 MaaFramework 或
MFAAvalonia 中的原生崩溃。
